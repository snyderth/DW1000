...
void DW1000Class::enableLedBlinking() {
	...
	// for(int i = 0; i < LEN_PMSC_LEDC; i++){
	// 	Serial.println(gpiomode[i], HEX);
	//
	// }
	//set GPIO in LED mode
	setBit(gpiomode, LEN_GPIO_MODE, MSGP0, 1);
	setBit(gpiomode, LEN_GPIO_MODE, MSGP1, 1);
	setBit(gpiomode, LEN_GPIO_MODE, MSGP2, 1);
	setBit(gpiomode, LEN_GPIO_MODE, MSGP3, 1);

	// Enable Blinking LEDs
	setBit(pmscledc, LEN_PMSC_LEDC, BLNKEN, 1);
	// for(int i = 0; i < LEN_PMSC_LEDC; i++){
	// 	Serial.println(gpiomode[i], DEC);
	//
	// }

	// Supposed to enable GPIO for LED output
	...
	//Make sure we set the bits correctly
	// readBytes(GPIO_CTRL, GPIO_MODE_SUB, gpiomode, LEN_GPIO_MODE);
	// for(int i = 0; i < LEN_PMSC_LEDC; i++){
	// 	Serial.println(gpiomode[i], DEC);
	// }

	//use the debounce clock to set the rate of the LED blinking

	// byte pmscctrl0[LEN_PMSC_CTRL0];
	//
	// readBytes(PMSC, PMSC_CTRL0_SUB, pmscctrl0, LEN_PMSC_CTRL0);
	//
	// setBit(pmscctrl0, LEN_PMSC_CTRL0, 23, 1);
	// setBit(pmscctrl0, LEN_PMSC_CTRL0, 18, 1);
	//
	// //Enable LED blinking, set rate
	// writeBytes(PMSC, PMSC_CTRL0_SUB, pmscctrl0, LEN_PMSC_CTRL0);
	//
	enableDebounceClock();

	//Configure the divider for the
	//LED Debounce clock (DW1000 User Manual page 198 version 2.11)
	byte pmscctrl1[LEN_PMSC_CTRL1];
	readBytes(PMSC, PMSC_CTRL1_SUB, pmscctrl1, LEN_PMSC_CTRL1);
	//This is the top 6 bits of a 10 bit counter that divides the
	//19.2 MHz clock by this number (max 2016), default is 31st bit
	// set
	setBit(pmscctrl1, LEN_PMSC_CTRL1, 31, 0);
	setBit(pmscctrl1, LEN_PMSC_CTRL1, 30, 1);
...
void DW1000Class::blinkAllLEDs(){
  byte pmscledc[LEN_PMSC_LEDC];
	...
	//DO a blink of the LEDs
	writeBytes(PMSC, PMSC_LEDC_SUB, pmscledc, LEN_PMSC_LEDC);


	setBit(pmscledc, LEN_PMSC_LEDC, 16, 0);
	setBit(pmscledc, LEN_PMSC_LEDC, 17, 0);
	setBit(pmscledc, LEN_PMSC_LEDC, 18, 0);
	setBit(pmscledc, LEN_PMSC_LEDC, 19, 0);
	writeBytes(PMSC, PMSC_LEDC_SUB, pmscledc, LEN_PMSC_LEDC);
}
...
void DW1000Class::setDefaults() {
	if(_deviceMode == TX_MODE) {
		...
		setFrameFilterAllowData(false);
		setFrameFilterAllowReserved(false);
...
uint16_t DW1000Class::getDataLength() {
	...
	if(_deviceMode == TX_MODE) {
		...
		// Serial.println("Attempting to access the RX info register...");
...
void DW1000Class::getData(String& data) {
	uint16_t i;
	Serial.println("Retrieving data length...");
	uint16_t n = getDataLength(); // number of bytes w/o the two FCS ones
	Serial.print("Data length: "); Serial.println(n);
	if(n <= 0) { // TODO
		return;
	}
	byte* dataBytes = (byte*)malloc(n);
